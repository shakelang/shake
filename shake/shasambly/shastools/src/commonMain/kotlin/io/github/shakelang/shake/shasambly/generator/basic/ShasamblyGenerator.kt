...
/**
 * Generator for primitive cast [Opcodes.PRIMITIVE_CAST]
 * Casts the top value on the stack from and to the given primitive type.
 */
class ShasamblyPrimitiveCastOpcode(val from: Int, val to: Int) : ShasamblyOpcode {
    override val size: Int get() = 1
    override fun generate(gen: ShasamblyGenerator): ByteArray {
        return byteArrayOf(Opcodes.PRIMITIVE_CAST, (from * 16 + to).toUByte().toByte())
    }
}