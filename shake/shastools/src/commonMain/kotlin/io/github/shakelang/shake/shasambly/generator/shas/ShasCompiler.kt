...
class ShasCompiler(private val input: CharacterInputStream) {
    ...
    private fun expectCommand() {
        ...
        when(cmd.toString().uppercase()) {
            ...
            "B_NEG" -> out.writeByte(Opcodes.B_NEG)
            "S_NEG" -> out.writeByte(Opcodes.S_NEG)
            "I_NEG" -> out.writeByte(Opcodes.I_NEG)
            "L_NEG" -> out.writeByte(Opcodes.L_NEG)
            "F_NEG" -> out.writeByte(Opcodes.F_NEG)
            "D_NEG" -> out.writeByte(Opcodes.D_NEG)
            "B_ABS" -> out.writeByte(Opcodes.B_ABS)
            "S_ABS" -> out.writeByte(Opcodes.S_ABS)
            "I_ABS" -> out.writeByte(Opcodes.I_ABS)
            "L_ABS" -> out.writeByte(Opcodes.L_ABS)
            "F_ABS" -> out.writeByte(Opcodes.F_ABS)
            "D_ABS" -> out.writeByte(Opcodes.D_ABS)
