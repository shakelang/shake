...
public class DoubleValue implements InterpreterValue {
    ...
    public <T extends InterpreterValue> T castTo(CastNode.CastTarget type) {
        if(type == CastNode.CastTarget.BYTE) return (T) new IntegerValue((int) this.getValue());
        if(type == CastNode.CastTarget.SHORT) return (T) new IntegerValue((int) this.getValue());
        if(type == CastNode.CastTarget.INTEGER) return (T) new IntegerValue((int) this.getValue());
        if(type == CastNode.CastTarget.LONG) return (T) new IntegerValue((int) this.getValue());
